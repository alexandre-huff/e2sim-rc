...
E2AP_PDU_t* e2ap_xml_to_pdu(char const* xml_message)
{
  ...
  logger_debug("Generate E2AP PDU from XML file: %s", XML_path);
  memset(buf, 0, sizeof(buf));

  f = fopen(XML_path, "r");
  if(!f){
     logger_error("Unable to open %s. Make sure you have set the Environment Variable E2SIM_DIR, see README", XML_path);
  }
  ...
  {
    logger_fatal("Input too long: %s", XML_path);
    exit(1);
  }
...
void e2ap_asn1c_decode_pdu(E2AP_PDU_t* pdu, unsigned char *buffer, int len)
{
  ...
  if (dec_ret.code != RC_OK) {
    logger_fatal("[E2AP ASN] Failed to decode pdu");
    exit(1);
  }
  else {
    logger_debug("[E2AP ASN] Decoded succesfully");
  }
}
...
int e2ap_asn1c_get_procedureCode(E2AP_PDU_t* pdu)
{
  ...
  switch(pdu->present)
  {
    ...
    default:
      logger_error("[E2AP] Unknown index %d in E2AP PDU", (int)pdu->present);
      break;
  }
